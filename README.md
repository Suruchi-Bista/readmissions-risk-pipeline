...
├── dashboard/                # Final analytics dashboard built on processed dataset
│   ├── hospital-readmission-risk-dashboard.pbix   # Power BI dashboard
│   └── hospital-readmission-risk-dashboard.pdf    # Exported dashboard
...
The cleaned dataset generated by the pipeline powers an interactive Power BI dashboard designed to explore hospital readmission risk across the United States.

The dashboard analyzes **5,421 hospitals** by combining CMS Hospital Readmissions data with the **CDC Social Vulnerability Index (SVI)** to understand how structural vulnerability and hospital performance influence readmission outcomes.

### Dashboard Pages

**National Overview**

Provides a high-level summary of readmission risk patterns across the United States including:

- Total hospitals analyzed
- Percentage of highest risk hospitals
- Geographic distribution of risk
- Community vulnerability breakdown

**Risk Drivers**

Explores the relationship between community vulnerability and readmission performance using:

- Scatter plot analysis of SVI vs Readmission Ratio
- Quadrant segmentation of risk categories
- Average readmission ratios by risk bucket
- State-level comparisons

**Hospital Explorer**

Allows hospital-level investigation through:

- Filters by state and risk category
- Hospital performance table
- Conditional formatting highlighting elevated readmission ratios
- KPI summaries for selected hospitals

### Key Insights

- **52% of hospitals serve high-vulnerability communities**
- **15% of hospitals fall into the highest risk category**
- Hospitals in the upper-right risk quadrant face both structural vulnerability and elevated readmission risk

### Dashboard Files

The dashboard files are located in the `dashboard/` directory:

- `hospital-readmission-risk-dashboard.pbix` — Power BI report
- `hospital-readmission-risk-dashboard.pdf` — exported dashboard version
